...
export default function FaqList({ variant = 'sub' }: Props) {
  const wrapperClass =
    variant === 'top'
      ...
      : 'u-faqList u-fade-in  max-w-[800px]  px-10 lg:px-0 mx-auto mt-10 lg:mt-0 mb-20 lg:mb-40'; // 下層ページ用
  ...
  return (
    <ul className={wrapperClass}>
      {displayFaqs.map((item) => (
        ...
        >
          <div className="lg:text-lg flex justify-start align-top gap-3 lg:gap-4 pb-2 lg:pb-6">
            <p className="text-2xl flex-shrink-0">Q</p>
            <p className='pt-1'>{item.question}</p>
          </div>
          <div className="flex justify-start align-top gap-3 lg:gap-4">
